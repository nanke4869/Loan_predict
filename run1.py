...
if __name__ == '__main__':
    ...
    X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=.33, random_state=2020)
    ...
    rf_grid = random_forest(X, y)
    rf_predict = rf_grid.predict_proba(test)[:, -1]
    np.savetxt("rf_predict.txt", rf_predict, fmt='%.06f')

    dectree_grid = decision_tree(X, y)
    dectree_predict = dectree_grid.predict_proba(test)[:, -1]
    np.savetxt("dectree_predict.txt", dectree_predict, fmt='%.06f')

    XGBClassifier = XGB(X, y, X_val, y_val)
    XGB_predict = XGBClassifier.predict_proba(test)[:, -1]
    np.savetxt("XGB_predict.txt", XGB_predict, fmt='%.06f')

    bagging_grid = bagging(X, y)
    bagging_predict = bagging_grid.predict_proba(test)[:, -1]
    np.savetxt("bagging_predict.txt", bagging_predict, fmt='%.06f')

    adaBoost_grid = AdaBoost(X, y)
    adaBoost_predict = adaBoost_grid.predict_proba(test)[:, -1]
    np.savetxt("adaBoost_predict.txt", adaBoost_predict, fmt='%.06f')

    ExtraTreesClassifier = extra_trees(X, y, X_val, y_val)
    ExtraTrees_predict = ExtraTreesClassifier.predict_proba(test)[:, -1]
    np.savetxt("ExtraTrees_predict.txt", ExtraTrees_predict, fmt='%.06f')

    voting_classifier = voting(X, y, X_val, y_val)
    voting_predict = voting_classifier.predict_proba(test)[:, -1]
    np.savetxt("voting_predict.txt", ExtraTrees_predict, fmt='%.06f')
